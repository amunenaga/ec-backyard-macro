...
Sub 読込集計_フルオート()
    'meisai/paymentMethodを読み込んで式も入れて、新規シートへ保存して終了
    '計3個のファイルが必要ですので、マクロ起動後のウィンドウで指定して下さい。
    'CSV:meisai,PaymentMethod Xlsx:価格チェック○月.xlsx
...
'価格チェックワークシートのコピーが終わったら

'原価チェックシートの商品コードを修正＞VlookupでヒットさせるためにStr型で格納し直す。
Worksheets("価格チェック").Activate
...
sh1EndRow = Sheet1.UsedRange.Rows.Count - 1

'合計、個数、件数など集計対象の列をダブル型にキャスト
On Error GoTo ErrorMes
   'Code列の特定
    Dim codeCol As Integer
    codeCol = WorksheetFunction.Match("Product Code", Sheet1.Range("A1").Resize(1, 20), 0)
    
On Error GoTo 0
    
    Dim Rng As Range
    Set Rng = Union(Sheet1.Cells(2, findColum("Quantity")).Resize(sh1EndRow, 1), _
                    Sheet1.Cells(2, findColum("Unit Price")).Resize(sh1EndRow, 1), _
                    Sheet1.Cells(2, findColum("Line Sub Total")).Resize(sh1EndRow, 1))
...
Set MeisaiCodeRange = Sheet1.Cells(1, codeCol).Resize(Sheet1.UsedRange.Rows.Count - 1, 1)
...
findColum = WorksheetFunction.Match(str, Sheet1.Range("A1").Resize(1, 20), 0)

End Function

Private Sub 転記と重複削除()
'売上商品の一意な表を用意します。
'meisaiのコードと商品名を集計シートに転記して重複削除します。
...
On Error GoTo ErrorMes
   'Code列の特定
    ...
    DescriptionCol = WorksheetFunction.Match("Description", Sheet1.Range("A1").Resize(1, 20), 0)
...
With Sheet4
    ...
    For i = 2 To Sheet1.UsedRange.Rows.Count
        .Cells(i + 1, 1).Value = Sheet1.Cells(i, DescriptionCol)
        .Cells(i + 1, 2).Value = Sheet1.Cells(i, codeCol)
    Next
...
With Sheet2
    ...
    'SUMIFで計算するのでセルの値はダブル型
    For i = 2 To EndRow
        .Cells(i, 8).NumberFormat = "0"
        .Cells(i, 8).Value = CDbl(Cells(i, 8).Value)
    Next
...
ResizeRow = Sheet4.Range("A2").End(xlDown).Row - 1

With Sheet4.Range("A2").Resize(ResizeRow, 9)
